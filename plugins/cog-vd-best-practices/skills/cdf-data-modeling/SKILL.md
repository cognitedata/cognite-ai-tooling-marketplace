...
Do not autonomously make these decisions — pause and present the options with a recommendation instead:
...
| Adding a reverse relation to an enterprise or CDM view | Couples the enterprise model to a solution's forward property name — developer must accept that dependency |
...
| Any schema change on a container in production with known consumers | Breaking or not — the developer must validate impact on live data pipelines |
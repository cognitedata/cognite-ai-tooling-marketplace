...
- **`btree`** ΓÇö for scalar and specific list types. Supports equality, range, and (when `cursorable: true`) ordered pagination. Optional `bySpace: true` prefixes the index with `node.space`, which is useful when queries are usually scoped to a single space.
- **`inverted`** ΓÇö for **list-type properties** (e.g. `text[]`, `int64[]`, `direct[]`). Enables `containsAny` / `containsAll` filtering across the elements of the list. **Inverted indexes cannot be `cursorable`**.
...
## Index Limits
- Max **10 indexes per container** (`usedFor: node`). Use slots strategically ΓÇö don't index everything.
- Never modify indexes or constraints on CDM types (`cdf_cdm` space) ΓÇö they are immutable.

## Default-indexed base properties

Every node and edge already has btree indexes on the following base properties ΓÇö you do not need (and cannot) add them yourself:
...
- Cursoring through **all instances scoped by `space`** is performant out of the box ΓÇö use it for full backfills.
- Filters on `type`, `startNode`, `endNode` are fast but **cannot cursor** ΓÇö pair them with a cursorable custom index if you need to page results.
- Filters on **`lastUpdatedTime`** and **`createdTime`** are **not** performant except on very small datasets. The Cognite docs explicitly warn against using them in DMS `/list` and `/query` filters. Do not design workflows that rely on `lastUpdatedTime` filtering ΓÇö use `/sync` (which subscribes to changes) instead. See `cdf-dms-queries` for the query-side pattern.

> Transformations note: The `is_new()` function inside CDF Transformations SQL uses `lastUpdatedTime` as the cursor argument. That is a *different* mechanism ΓÇö it is not a DMS filter, so the warning above does not apply there. See the `cognite-transformation` skill.
...
### Composite Indexes
Create composite indexes for common query patterns ΓÇö especially **status + timestamp** combinations. This avoids scanning all records of one status to find a time range.
...
- **`text[]` cannot have a btree index** ΓÇö use an `inverted` index instead.
...
- If `maxListSize` is not set or exceeds these caps, CDF rejects the btree index. Either lower `maxListSize`, remove the index, or use an `inverted` index (for `text[]`) ΓÇö and document the trade-off.
...
- Text properties with values larger than 2400 bytes cannot be btree-indexed ΓÇö split the property, cap `maxTextSize`, or index a hashed/derived column instead.
...
## What NOT to Index
- Properties already covered by unique constraints (e.g., `tag_number`) ΓÇö these function as indexes.
- Properties never used in filters or sorts.
- CDM container properties ΓÇö those are managed by Cognite.
...
workOrderOperations:
  through:
    ...
    identifier: functionalLocation   # ΓåÉ this property must be indexed
...
**Do not** add unrelated `requires` constraints just to silence warnings ΓÇö they are ingest-time dependencies and will make writes fail if the required container is absent. See `cdf-data-model-structure.md` ΓåÆ *Query Optimization via `requires`* for the semantic-validity rule.
...
## Composite Indexes ΓÇö Order Matters
...
- Do not oversize: a small number of well-chosen composite indexes beats many broad ones ΓÇö each additional index slows every ingest.
...
## Debug notices ΓÇö closing the loop from queries to indexes

The Cognite docs expose a first-class debugging tool for `/query` and `/sync` ΓÇö enable with `debug: {}` on the request. The notices most relevant to indexing are:

- **`sortNotBackedByIndex`** ΓÇö the query's sort has no cursorable index; add one whose property order matches the sort.
- **`unindexedThrough`** ΓÇö a `through` traversal targets a non-indexed direct-relation property; add a btree index on that property.
- **`significantHasDataFiltering`** ΓÇö a `hasData` filter over a multi-container view forces per-container joins; add `requires` constraints so the planner can shortcut them.
- **`significantPostFiltering`** ΓÇö filters are too late in the query pipeline; move selective filters earlier (this is a query-shape problem, not an index one).
